...
export function toolCallLine(theme: Theme, title: string, suffix?: string): string {
  if (!suffix) {
    return theme.fg("toolTitle", theme.bold(title));
  }

  return `${theme.fg("toolTitle", theme.bold(`${title} `))}${suffix}`;
}

export function renderToolCall(theme: Theme, title: string, suffix?: string): Text {
  return new Text(toolCallLine(theme, title, suffix), 0, 0);
}
