...
type AutoBoomerangMode = "off" | "once" | "always";

const BOOMERANG_EDITOR_STATUS_KEY = "boomerang";
const BOOMERANG_EDITOR_STATUS_TEXT = String.fromCodePoint(0x1fa83);
const BOOMERANG_EDITOR_STATUS_ONCE_TEXT = `${BOOMERANG_EDITOR_STATUS_TEXT}+1`;
const BOOMERANG_EDITOR_STATUS_ALWAYS_TEXT = `${BOOMERANG_EDITOR_STATUS_TEXT}+∞`;
let branchSummaryPatchInstalled = false;
let commandContextPatchInstalled = false;

type CommandCapableContext = ExtensionContext &
  Pick<ExtensionCommandContext, "navigateTree" | "reload">;

function isCommandCapableContext(ctx: ExtensionContext): ctx is CommandCapableContext {
  const candidate = ctx as Partial<CommandCapableContext>;
  return typeof candidate.navigateTree === "function" && typeof candidate.reload === "function";
}

function installCommandCapableShortcutContextPatch(): void {
  if (commandContextPatchInstalled) return;
  commandContextPatchInstalled = true;

  const prototype = ExtensionRunner.prototype as unknown as {
    createContext?: () => ExtensionContext;
  };
  const originalCreateContext = prototype.createContext;
  if (typeof originalCreateContext !== "function") return;

  prototype.createContext = function patchedBoomerangCreateContext(this: unknown): ExtensionContext {
    const context = originalCreateContext.call(this);
    const runner = this as {
      assertActive?: () => void;
      navigateTreeHandler?: ExtensionCommandContext["navigateTree"];
      reloadHandler?: ExtensionCommandContext["reload"];
    };
...
function getBoomerangEditorStatusText(
  active: boolean,
  autoMode: AutoBoomerangMode,
): string | null {
  if (autoMode === "always") return BOOMERANG_EDITOR_STATUS_ALWAYS_TEXT;
  if (autoMode === "once" && !active) return BOOMERANG_EDITOR_STATUS_ONCE_TEXT;
  if (active) return BOOMERANG_EDITOR_STATUS_TEXT;
  return null;
}

function getBoomerangBranchTask(message: unknown): string | undefined {
  const task = (message as { details?: { task?: unknown } } | undefined)?.details?.task;
  return typeof task === "string" && task.trim() ? task.trim() : undefined;
}

export function installBoomerangBranchSummaryRendererPatch(): void {
  if (branchSummaryPatchInstalled) return;
  branchSummaryPatchInstalled = true;

  const prototype = BranchSummaryMessageComponent.prototype as unknown as {
    updateDisplay?: (this: unknown) => void;
  };
  const originalUpdateDisplay = prototype.updateDisplay;
  if (!originalUpdateDisplay) return;

  prototype.updateDisplay = function patchedBoomerangBranchSummaryDisplay(this: unknown): void {
    ...
    const task = getBoomerangBranchTask(component.message);
    if (!task) return;

    const summaryText = component.children
      ?.slice()
      .reverse()
      .find((child) => typeof child.setText === "function");
    if (summaryText?.setText) {
      summaryText.setText(`${task} (${keyText("app.tools.expand" as never)} to expand)`);
    }
...
export function extractRethrow(task: string): {
  ...
  while (i < mainSegment.length) {
    ...
    if (token === "--rethrow" && !hasRethrowToken) {
      ...
      if (
        lookahead < mainSegment.length &&
        mainSegment[lookahead] !== '"' &&
        mainSegment[lookahead] !== "'"
      ) {
...
export default function (pi: ExtensionAPI) {
  ...
  let autoBoomerangMode: AutoBoomerangMode = "off";
  ...
  function parseTemplateFile(filePath: string): PromptTemplate {
    ...
    const thinking =
      thinkingRaw && (VALID_THINKING_LEVELS as readonly string[]).includes(thinkingRaw)
  ...
  function resolveModel(modelSpec: string, ctx: ExtensionContext): Model<any> | undefined {
    ...
    const availableMatches = ctx.modelRegistry
      .getAvailable()
      .filter((model) => model.id === modelSpec);
  ...
  async function restoreModelAndThinking(ctx: ExtensionContext): Promise<void> {
    ...
    if (previousModel) {
      ...
      try {
        ...
        restoreErrors.push(
          `model:${modelToRestore.provider}/${modelToRestore.id} (${String(error)})`,
        );
      }
    }
  ...
  function hasAssistantMessageAfterEntry(entries: SessionEntry[], entryId: string | null): boolean {
    const startIndex = entryId === null ? -1 : entries.findIndex((entry) => entry.id === entryId);
  ...
  async function executeChainStep(ctx: ExtensionContext): Promise<void> {
    ...
    if (isLastStep) {
      const allModels = chainState.configHistory.map((c) => c.model).filter(Boolean) as string[];
      const allSkills = chainState.configHistory.map((c) => c.skill).filter(Boolean) as string[];
    ...
    try {
      for (let i = 1; i <= totalRethrows; i++) {
          ...
          if (!parsed) {
            ...
            for (let stepIndex = 0; stepIndex < parsed.steps.length; stepIndex++) {
              ...
              if (template.models.length > 0) {
                ...
                if (!result) {
                  ctx.ui.notify(
                    `Chain aborted: couldn't switch model for step ${stepIndex + 1}`,
                    "error",
                  );
                  aborted = true;
                  break;
              ...
              ctx.ui.setStatus(
                ...
                ctx.ui.theme.fg(
                  "warning",
                  `rethrow ${i}/${totalRethrows}, chain ${stepIndex + 1}/${parsed.steps.length}`,
                ),
              );
              ...
              pi.sendUserMessage(expandedContent);
              const turnStarted = await waitForTurnStart(
                ctx,
                () => boomerangActive && rethrowState !== null,
              );
            ...
            const lastThinking = configHistory
              .map((c) => c.thinking)
              .filter(Boolean)
              .pop();
        ...
        try {
          ...
          collapseResult = await commandCtx.navigateTree(currentRethrow.autoAnchorId, {
            summarize: true,
          });
        } catch (err) {
      ...
      const completedNormally =
        rethrowState !== null && boomerangActive && completedRethrows === totalRethrows;
  ...
  async function reloadFallbackChatDisplay(ctx: ExtensionContext): Promise<void> {
    ...
    const ctxReload = (ctx as ExtensionContext & { reload?: () => Promise<void> }).reload;
    const reloadDisplay = reloadFallbackDisplay ?? (typeof ctxReload === "function" ? ctxReload : null);
    if (!reloadDisplay) return;

    try {
      await reloadDisplay();
    } catch (error) {
      ctx.ui.notify(
        `Boomerang summary created, but chat reload failed: ${String(error)}`,
        "warning",
      );
    }
  }
  ...
  function updateStatus(ctx: ExtensionContext) {
    const editorStatusActive = Boolean(
      boomerangActive || pendingCollapse || rethrowState || chainState || toolCollapsePending || toolQueuedTask,
    );
    const editorStatusText = getBoomerangEditorStatusText(editorStatusActive, autoBoomerangMode);
    if (editorStatusText) {
      pi.events.emit(EDITOR_SET_STATUS_SEGMENT_EVENT, {
        key: BOOMERANG_EDITOR_STATUS_KEY,
        text: editorStatusText,
        align: "right",
        priority: -1,
      });
    } else {
      pi.events.emit(EDITOR_REMOVE_STATUS_SEGMENT_EVENT, { key: BOOMERANG_EDITOR_STATUS_KEY });
    }
    ...
    } else if (autoBoomerangMode === "once") {
      ctx.ui.setStatus("boomerang", ctx.ui.theme.fg("accent", "auto"));
    } else if (autoBoomerangMode === "always") {
      ctx.ui.setStatus("boomerang", ctx.ui.theme.fg("accent", "always"));
  ...
  function describeAutoBoomerangMode(mode: AutoBoomerangMode): string {
    switch (mode) {
      case "once":
        return "one-shot";
      case "always":
  ...
  function cycleAutoBoomerangMode(mode: AutoBoomerangMode): AutoBoomerangMode {
    switch (mode) {
      case "off":
        return "once";
      case "once":
  ...
  function setAutoBoomerang(mode: AutoBoomerangMode, ctx: ExtensionContext): void {
    autoBoomerangMode = mode;
    updateStatus(ctx);
    ctx.ui.notify(`Auto-boomerang ${describeAutoBoomerangMode(mode)}.`, "info");
  }
  ...
  function isValidationCommand(command: string): boolean {
    return (
      /(^|\s)(npm|pnpm|yarn|bun)\s+(run\s+)?(test|check|lint|typecheck|verify)\b/.test(command) ||
  ...
  function collectBoomerangSummaryDetails(
    entries: SessionEntry[],
    task: string,
  ): BoomerangSummaryDetails & { outcome: string } {
  ...
  function generateSummaryFromEntries(
    ...
    if (details.validationCommands.length > 0) {
      commandLines.push(
        `- Validation: ${details.validationCommands.map((command) => `\`${command}\``).join(", ")}`,
      );
    }
    commandLines.push(
  ...
  async function startTask(
    ...
    if (extracted) {
      ...
      if (!chainParsed && isTemplate) {
        ...
        rethrowState.templateRef =
          spaceIndex > 0 ? taskString.slice(1, spaceIndex) : taskString.slice(1);
      ...
      if (ignoredLoopAlias) {
        ctx.ui.notify(
          `Ignored --loop because --rethrow is set. Using --rethrow ${extracted.rethrowCount}.`,
          "info",
        );
      }
    ...
    if (isTemplate) {
      ...
      const templateRef = spaceIndex > 0 ? trimmed.slice(1, spaceIndex) : trimmed.slice(1);
      const templateArgs = spaceIndex > 0 ? trimmed.slice(spaceIndex + 1) : "";
  ...
  pi.registerCommand("boomerang", {
    ...
    handler: async (args, ctx) => {
      ...
      if (trimmed === "auto" || trimmed.startsWith("auto ")) {
        ...
        if (mode === "on") {
          setAutoBoomerang("once", ctx);
        } else if (mode === "always") {
          setAutoBoomerang("always", ctx);
        } else if (mode === "off") {
          setAutoBoomerang("off", ctx);
        } else if (mode === "toggle") {
          setAutoBoomerang(cycleAutoBoomerangMode(autoBoomerangMode), ctx);
        } else if (mode === "status") {
          ctx.ui.notify(`Auto-boomerang is ${describeAutoBoomerangMode(autoBoomerangMode)}.`, "info");
        } else {
          ctx.ui.notify("Usage: /boomerang auto [on|always|off|toggle|status]", "error");
        }
        return;
      }
      ...
      if (!trimmed) {
          ctx.ui.notify(
          "Usage: /boomerang <task> | auto [on|always|off|toggle|status] | anchor | tool [on|off] | guidance [text|clear]",
          "error",
        );
        return;
      }
  ...
  pi.registerShortcut("ctrl+alt+b", {
    description: "Cycle auto-boomerang mode",
    handler: async (ctx) => {
      if (isCommandCapableContext(ctx)) {
        storedCommandCtx = ctx as ExtensionCommandContext;
        reloadFallbackDisplay = () => ctx.reload();
      }
      setAutoBoomerang(cycleAutoBoomerangMode(autoBoomerangMode), ctx);
    },
  ...
  pi.on("input", async (event, ctx) => {
    autoBoomerangCandidate = null;
    if (autoBoomerangMode === "off") return;
    if (
      boomerangActive ||
      pendingCollapse ||
      rethrowState ||
    ...
    pi.registerTool({
      ...
      parameters: Type.Object({
        task: Type.Optional(
          Type.String({
            description:
              "Task to execute. Supports --rethrow N for multi-pass execution with summaries between rethrows.",
          }),
        ),
      }),
      execute: async (_id, params, _signal, _onUpdate, ctx) => {
        if (!toolEnabled) {
          return {
            content: [
              {
                type: "text",
                text: "Boomerang tool is disabled. User must run `/boomerang tool on` to enable.",
              },
            ],
            details: {},
          };
        ...
        if (boomerangActive) {
          return {
            content: [
              { type: "text", text: "A boomerang is already active. Wait for it to complete." },
            ],
            details: {},
          };
        }
        ...
        if (task) {
          if (!storedCommandCtx) {
            return {
              content: [
                {
                  type: "text",
                  text: "No command context. Run any /boomerang command first to initialize.",
                },
              ],
              details: {},
              isError: true,
          ...
          if (toolQueuedTask) {
            return {
              content: [
                {
                  type: "text",
                  text: "A boomerang task is already queued. Wait for it to start before queueing another task.",
                },
              ],
              details: {},
              isError: true,
          ...
          return {
            content: [
              {
                type: "text",
                text: `Task queued: "${task}". Will start autonomously when this turn ends.`,
              },
            ],
            details: {},
          };
        ...
        if (toolAnchorEntryId === null) {
          ...
          return {
            content: [
              {
                type: "text",
                text: "Boomerang anchor set. Do your work, then call boomerang again to summarize the context.",
              },
            ],
            details: {},
          };
        ...
        return {
          content: [
            {
              type: "text",
              text: "Boomerang complete. Context will be summarized when this turn ends.",
            },
          ],
          details: {},
        };
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    if (toolQueuedTask && storedCommandCtx) {
      ...
      toolQueuedTask = null;
      await startTask(task, storedCommandCtx, {
        model: ctx.model,
    ...
    if (toolCollapsePending && toolAnchorEntryId) {
      ...
      if (!storedCommandCtx) {
        ...
        try {
          const entryId = sm.branchWithSummary(
            toolAnchorEntryId,
            generatedSummary.summary,
            generatedSummary.details,
          );
    ...
    if (boomerangActive && autoFallbackCollapse) {
      ...
      const branch = sm.getBranch();
      const startIndex =
        fallbackCollapse.targetId === null
          ? -1
  ...
  pi.on("session_before_tree", async (event) => {
    ...
    const rethrowInfo = activeRethrowState
      ? {
          rethrow: activeRethrowState.currentRethrow,
          totalRethrows: activeRethrowState.rethrowCount,
        }
      : undefined;
    const generatedSummary = generateSummaryFromEntries(
      entries,
    ...
    const isRethrowCollapse =
      activeRethrowState !== null && pendingCollapse.targetId === activeRethrowState.autoAnchorId;
