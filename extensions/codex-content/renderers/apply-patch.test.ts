...
test("summarizeApplyPatchResult uses specific verbs for single-file operations", () => {
  assert.deepEqual(
    summarizeApplyPatchResult({ affected: { added: ["src/new.ts"] } }),
    { title: "Added", suffix: "src/new.ts" },
  );
  assert.deepEqual(
    summarizeApplyPatchResult({ affected: { modified: ["src/app.ts"] } }),
    { title: "Modified", suffix: "src/app.ts" },
  );
  assert.deepEqual(
    summarizeApplyPatchResult({ affected: { deleted: ["src/old.ts"] } }),
    { title: "Deleted", suffix: "src/old.ts" },
  );
});
...
test("summarizeApplyPatchResult includes per-file stats for multi-file details", () => {
  ...
  assert.deepEqual(
    summarizeApplyPatchResult({
      files: [
        { action: "added", path: path.join(cwd, "src", "inside.ts"), diff: "+ inside" },
        { action: "deleted", path: "/tmp/outside.ts", diff: "- outside" },
      ],
    }),
    { title: "Patched", suffix: "src/inside.ts (+1 -0), /tmp/outside.ts (+0 -1)" },
  );
});
  ...
  const component = renderApplyPatchResult(
    ...
    } as any,
    {
      ...
      details: {
        ...
        files: [{ action: "added", path: "/tmp/demo.txt", diff: "+ apply_patch demo\n+ render check" }],
      },
      isError: false,
    } as any,
    true,
    ...
    } as any,
    {
      ...
      details: {
        ...
        files: [
          ...
          { action: "added", path: "/tmp/pi-apply-patch-outside.txt", diff: "+ outside" },
        ],
      },
      isError: false,
    } as any,
...
test("renderApplyPatchResult shows compact A/M/D file sections when expanded for multi-file patches", () => {
  ...
  const component = renderApplyPatchResult(
    ...
    } as any,
    {
      ...
      details: {
        exitCode: 0,
        affected: { added: [insidePath], modified: [insidePath], deleted: [insidePath] },
        files: [
          { action: "added", path: insidePath, diff: "+ inside" },
